...
var digging_blocks = {} # Słownik do śledzenia stanu kopanych bloków: {Vector2i(map_coords): current_durability}
var digging_timer = null # Timer do kontrolowania szybkości kopania
var digging_interval = 0.4 # Częstotliwość "uderzeń" w sekundach
var digging_target = null # Aktualne koordynaty kopanego bloku
var digging_damage = 25.0 # Ile wytrzymałości zabiera każde "uderzenie"
...
func _input(event: InputEvent) -> void:
    if event.is_action_pressed("dig"):
        ...
        # Zatrzymaj kopanie po puszczeniu przycisku
        stop_digging()
...
func _on_digging_timer_timeout() -> void:
    ...
    # Kontynuuj kopanie aktualnego bloku
...
func handle_digging():
    ...
    # --- WARUNEK ZASIĘGU: Pole gracza LUB 4 sąsiednie ---
    var is_valid_target = (
        ...
        if not (dx == 0 and dy == 0): # Nie próbuj kopać pod sobą
            ...
            if tile_data:
                ...
                if is_diggable:
                    ...
                    # Ustaw cel kopania i rozpocznij proces
                    start_digging(target_map_coords)
                else:
                    print("Tile at ", target_map_coords, " is not diggable.")
                    stop_digging()
            else:
                # W tej komórce nie ma ani drabiny, ani kafelka
...
func dig_block_progress(map_coords: Vector2i) -> void:
    ...
    # Efekty wizualne kopania (np. cząsteczki)
    # spawn_digging_particles(map_coords)
    
    print("Digging block at ", map_coords, " - Durability: ", digging_blocks[map_coords])
    
    # Sprawdź czy blok został zniszczony
    if digging_blocks[map_coords] <= 0:
        # Usuń blok z mapy
        ground_tilemap.set_cell(map_coords, -1)
        digging_blocks.erase(map_coords)
        # Odtwórz dźwięk zniszczenia bloku (jeśli istnieje)
        # TODO: $DiggingAudio.play()
        
        # Zatrzymaj kopanie
        stop_digging()

# Dodaj funkcję do tworzenia efektów cząsteczkowych (opcjonalne)
# func spawn_digging_particles(map_coords: Vector2i) -> void:
    # Tutaj możesz dodać kod do tworzenia efektów cząsteczkowych w miejscu kopania
    # Na przykład:
    # var particles = digging_particles_scene.instantiate()
    # particles.position = ground_tilemap.map_to_local(map_coords)
