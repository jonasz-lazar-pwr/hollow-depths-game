...
func _ready() -> void:
	...
	print("Ladder ready at: ", global_position)
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_RIGHT and event.pressed:
		handle_ladder_placement(event.position)
...
func handle_ladder_placement(click_pos: Vector2) -> void:
	if not ground_tilemap or not ladder_scene:
		return

	var target_coords = ground_tilemap.local_to_map(click_pos)
	var player_coords = ground_tilemap.local_to_map(global_position)

	print("Kliknięcie (map coords): ", target_coords, ", Gracz (map coords): ", player_coords)

	# Tu możesz już liczyć dx/dy względem gracza
	var ladder_pos = ground_tilemap.map_to_local(target_coords)

	var ladder_instance = ladder_scene.instantiate()
	ladder_instance.position = ladder_pos
	get_parent().add_child(ladder_instance)

	print("Postawiono drabinę!")
